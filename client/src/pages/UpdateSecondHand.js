...
export default function UpdateSecondHand() {
  ...
  useEffect(() => {
    ...
    const fetchSecondHand = async () => {
      ...
      if (data.success === false) {
        ...
        setFormDataSecondHand(data);
        setCount(0);
        setDescription(data.description);
        setPickSecondHand(data.pickSecondHand);
        setSelectedSecondHandType(data.secondhandcategory);
  ...
  const handleSubmit = async () => {
    try {
      ...
      setError(false);
      const res = await fetch(
        `/api/secondhand/update/${params.secondHandId}`,
        {
          method: "POST",
          credentials: "include",
          headers: {
        ...
        {isThird ? (
          <div className="flex flex-col gap-1">
            ...
            <div className="flex  gap-6">
              {formDataSecondHand.action &&
              formDataSecondHand.action === "sale" ? (
                <>
